 ...
 * 
 */
public class CpuOpponentO extends ComputerOpponent {

	/**
	 * Creates a new "O" computer opponent logic handler
	 * @param game
	 */
	public CpuOpponentO(SOSGame game) {
		super(game);
	}
	
	/**
	 * Chooses a board cell for computer opponent to place letter 'O'
	 * 
	 * @param boardSize the size of the board
	 * @param player the player before the computer opponent's turn, 'BLUE' or 'RED'
	 */
	public int[] computerChooseStrategy(int boardSize, SOSGame.Player player){
		...
		for(int row = 0; row < boardSize; row++) {
			for(int col = 0; col < boardSize; col++) {
				if(game.getBoard()[row][col] == ' ') {
					int score = game.checkScore(row, col, 'O', false);
					options[index][0] = row;
					options[index][1] = col;
		...
		game.makeMove(options[highestIndex][0], options[highestIndex][1], 'O');
